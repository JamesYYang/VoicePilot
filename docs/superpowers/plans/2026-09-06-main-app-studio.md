...
**待设计（2026-09-06 用户提出，未展开）**：主应用直接语音录入。当前听写链路绑死在悬浮条（采集在悬浮条渲染进程、状态机只 emit 给悬浮条窗口）。要支持「在主应用里开口录、结果进编辑器」，需先设计：快捷键在「主应用开着/没开」两种情况下走不同路径、两个窗口同时想录入时听谁的、结果落点（进编辑器/追加/替换）、采集实例归属。连同托盘打开主应用时顺带关悬浮条这条一起定。
