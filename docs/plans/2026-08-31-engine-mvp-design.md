...
> **2026-09-13 实施时的裁定**：本节是 2026-08-31 的原始设计。2026-09-13 落地「采纳写回」时**只采纳了「剪贴板 + 模拟 Ctrl/Cmd+V」这条主干**，下面标注的两条被明确**未采纳**。现行设计以 `docs/superpowers/specs/2026-09-13-adopt-injection-design.md` 为准。

- 默认剪贴板 + 模拟 Ctrl+V，注入后**恢复用户原剪贴板内容**
  - ⚠️ **本实现未采纳（2026-09-13）** —— 改为**从不还原**，写进剪贴板后就留着。原因：无从判断目标应用何时消费完粘贴，任何自动还原时机都是竞态；而「不还原」把失败的后果从「文本丢了」降级为「没省一步」（文本仍在剪贴板里，可直接手动粘），并与 2026-09-06 F3「复制不恢复」的既有原则一致。
- 终端类应用回退 `SendInput` 逐字注入（中文需 `KEYEVENTF_UNICODE`）；注入失败 → 悬浮条提示手动粘贴
  - ⚠️ **本实现未采纳（前半分句，2026-09-13）** —— 终端**不做逐字注入回退，直接走失败回退**（提示「已复制，请手动粘贴」）。原因：逐字注入要处理按键/字符映射与 IME 组字态，成本远高于收益，而终端粘贴行为不一致已列为本期**明确接受的代价**。后半分句「注入失败 → 提示手动粘贴」已采纳。
- 三个已知坑：Ctrl+V 后须等目标应用处理完再恢复剪贴板；恢复要枚举还原所有 format；中文 IME 组字态会吞掉 Ctrl+V，注入前需检测并清空合成状态
  - 第 1、2 条随「剪贴板不还原」一并消失；第 3 条（中文 IME 组字态）**本期不处理**，同样列为已接受的代价。
